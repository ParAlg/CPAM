...
void ANN(parlay::sequence<Tvec_point<T>*> v, int maxDeg, int beamSize,
  ...
  parlay::internal::timer t("ANN", report_stats);
  {
    ...
    build_t.start();
    size_t n = v.size();
    parlay::sequence<node_id> all_inserts = parlay::tabulate(n, [&] (size_t i) { 
        return static_cast<node_id>(i); });
    parlay::sequence<node_id> init_inserts = parlay::tabulate(n/2, [&] (size_t i) { 
        return static_cast<node_id>(i); });
    parlay::sequence<node_id> dynamic_inserts = parlay::tabulate(n/10, [&] (size_t i) { 
        return static_cast<node_id>((n-n/10)+i); });
    I.build_index(all_inserts);
    build_t.next("Build time");
    I.lazy_delete(dynamic_inserts);
    I.consolidate_deletes();
    I.print_graph_status();
    build_t.next("Delete time");
    I.insert(dynamic_inserts);
    I.print_graph_status();
    build_t.next("Insert time");
...
void ANN(parlay::sequence<Tvec_point<T>*> v, int maxDeg, int beamSize,
  ...
  parlay::internal::timer t("ANN", report_stats);
  {
    ...
    parlay::sequence<node_id> init_inserts = parlay::tabulate(n/2, [&] (size_t i) { 
        return static_cast<node_id>(i); });
    parlay::sequence<node_id> dynamic_inserts = parlay::tabulate(n/2, [&] (size_t i) { 
        return static_cast<node_id>(n/2+i); });
    I.build_index(all_inserts);
    build_t.next("Build time");
    I.lazy_delete(dynamic_inserts);
    I.consolidate_deletes();
