...
//    mapt.next("Map all edges noop time");
  ...
  size_t size_in_bytes() {
    ...
    cout << "Total vertices = " << total_vertices << " vertex_list_bytes = " << (total_vertices*56) << endl;
    cout << "Overall bytes = " << (edge_list_bytes + (total_vertices*56)) << std::endl;
...
//    free(v2_edges); free(tu_and_deg);
  ...
  auto insert_edges_batch(size_t m, tuple<uintV, uintV>* edges, bool sorted=false, bool remove_dups=false, size_t nn=std::numeric_limits<size_t>::max(), bool run_seq=false) const {
    ...
    if (remove_dups) {
      ...
      m = E.size();
      E_alloc = E.to_array();
      std::cout << "Remove dups: orig size = " << E_orig.size() << " new size = " << m << std::endl;
    ...
    t.next("prepare time");
    std::cout << "Num starts = " << num_starts << " E.size = " << E.size() << std::endl;
