...
struct traversable_graph : private graph {
  ...
  auto edge_map_dense(vertex_subset& vs, F f, const flags fl=0) {
    timer dt; dt.start();
    cout << "Running dense, vs.size = " << vs.size() << endl;
    timer tt; tt.start();
    vs.to_dense();
    tt.next("conversion time");
    size_t n = vs.n;
    auto prev = vs.d;
...
//      exit(0);
  ...
  auto edge_map_dense(vertex_subset& vs, F f, pbbs::sequence<edge_struct> const &vtxs, const flags fl=0) {
    cout << "Running dense (flatsnap)" << endl;
    vs.to_dense();
    size_t n = vs.n;
    auto prev = vs.d;
  ...
  auto edge_map(vertex_subset& vs, F f, timer& sparse_t, timer& dense_t, timer& fetch_t, timer& other_t, const flags fl=0) {
    ...
    if (fl & stay_dense && (vs.is_dense && vs.size() > (n/10))) {
      std::cout << "Dense (shortcut)" << std::endl;
      dense_t.start();
    ...
    cd.stop(); cd.reportTotal("calculate degree");
    std::cout << "Calculated for vs.size = " << vs.size() << std::endl;
    other_t.stop();
    if (total_edges + vs.size() > threshold) {
      std::cout << "Dense: " << (total_edges + vs.size()) << " " <<
        threshold << std::endl;
      dense_t.start();
      auto ret = edge_map_dense(vs, f, fl);
      dense_t.stop();
