...
void parallel_updates(commandLine& P) {
  ...
  for (size_t us=start; us<update_sizes.size(); us++) {
    ...
    for (size_t ts=0; ts<n_trials; ts++) {
      ...
      std::cout << "sizeof uintV = " << sizeof(uintV) << std::endl;
      std::cout << "ith_rand 0 = " << r.ith_rand(0) << std::endl;
      std::cout << "nn = " << nn << std::endl;
      for (size_t i=0; i<10; i++) {
        std::cout << std::get<0>(rmat(i)) << " " << std::get<1>(rmat(i)) << std::endl;
      }
...

      {
        ...
        // cout << "batch time = " << batch_time << " num_edges = " << VG.num_edges() << endl;
