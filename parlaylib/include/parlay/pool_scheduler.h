...
struct scheduler {
 public:
  ...
  unsigned int thread_id_start;
  bool include_self;

  static thread_local unsigned int thread_id;

  scheduler(size_t num_threads, unsigned int thread_id_start, void* fork_join_sched, bool include_self=false)
    ...
    std::cout << "Num_deques = " << num_deques << std::endl;
    ...
    for (unsigned int i = 1; i < num_threads; i++) {
      spawned_threads.emplace_back([&, i, finished, fork_join_sched]() {
  ...
  ~scheduler() {
    ...
    for (unsigned int i = 1; i < threads_to_spawn; i++) {
...
#pragma warning(disable : 4996)  // 'getenv': This function or variable may be unsafe.
  ...
  unsigned int init_num_workers() {
    std::cout << "Unexpected call to init_num_workers. Exiting." << std::endl;
    exit(0);
...
#pragma warning(pop)
  ...
  unsigned int worker_id() { return thread_id - thread_id_start; }
  unsigned int global_worker_id() { return thread_id; }
...
class fork_join_scheduler {
 ...
 public:
  fork_join_scheduler(size_t num_threads, int thread_id_start, bool include_self=false) : sched(std::make_unique<scheduler<Job>>(num_threads, thread_id_start, (void*)this, include_self)) {
    std::cout << "Made fork_join_pool_scheduler!" << std::endl;
  }

  unsigned int num_workers() { return sched->num_workers(); }
  unsigned int worker_id() { return sched->worker_id(); }
  unsigned int global_worker_id() { return sched->global_worker_id(); }
