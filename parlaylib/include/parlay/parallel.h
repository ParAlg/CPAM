...
inline size_t num_workers() {
  ...
  std::cout << " Called my num_workers." << std::endl;
...
inline size_t worker_id() {
  fork_join_scheduler* fj = (fork_join_scheduler*)fork_join_sched_ptr;
  if (fj) return fj->global_worker_id();
  return 0;
  //return internal::get_default_scheduler().worker_id();
}
...
inline void parallel_for(size_t start, size_t end, F f,
  ...
  if (end > start) {
    fork_join_scheduler* fj = (fork_join_scheduler*)fork_join_sched_ptr;
    if (fj) fj->parfor(start, end, f, (size_t) granularity, conservative);
...
inline void par_do(Lf left, Rf right, bool conservative) {
  fork_join_scheduler* fj = (fork_join_scheduler*)fork_join_sched_ptr;
  if (fj) return fj->pardo(left, right, conservative);
  left(); right();
}
