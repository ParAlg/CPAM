...
struct augmented_ops : Map {
  ...
  static void aug_sum_right(node* b, const K& key, aug& a) {
    while (b) {
      // std::cout << "Size b = " << Map::size(b) << " aug_stats: " << b->entry.second.size() << " " << b->entry.second.ref_cnt() << std::endl;
      if (!Map::comp(Map::get_key(b), key)) {
        // std::cout << "key (gt) = " << Map::get_key(b).first << " " << Map::get_key(b).second << std::endl;
	a.add_entry(Map::get_entry(b));
	if (b->rc) a.add_aug_val(b->rc->entry.second);
	b = b->lc;
      } else {
        // std::cout << "key (lt) = " << Map::get_key(b).first << " " << Map::get_key(b).second << std::endl;
        b = b->rc;
      }
    }
  }
  ...
  template<typename F>
  static void iterate_aug(node* a, const F& f) {
    if (!a) return;
    iterate_aug(a->lc, f);
